...
function SideAccordion({ icon, title, expanded, value, className, children }: { title: string, icon: React.ReactElement, value: string, expanded: boolean, className?: string, children: React.ReactNode }) {
  return (
    <>
      <AccordionItem className={cn('border-0 flex flex-col w-full',className)} value={value}>
  ...
  return (
    <aside style={{ width: expanded ? '280px' : '72px' }} className={`fixed flex flex-col bg-background justify-between items-center h-full py-10 inset-y-0 right-0 z-10 w-[72px] border-l border-secondary transition-all ${expanded ? "shadow-2xl px-6" : "px-0"}`}>
      ...
      <Accordion onValueChange={(e) => {expanded ? setAccordionOpen(e) : setExpanded(true); setAccordionOpen(e)}} value={accordionOpen} className={`flex flex-col overflow-hidden items-center gap-6 w-full ${expanded ? 'justify-start' : ''}`} type="single" collapsible>
        ...
        <SideAccordion className='max-h-[70%]' icon={<Activity className='w-8 h-8 text-primary' />} title='AÇÕES' expanded={expanded} value='2'>
          <ActionView />
        </SideAccordion>
        <SideAccordion className='max-h-[70%]' icon={<CircleEllipsis className='w-8 h-8 text-primary' />} title='COMPLEMENTO' expanded={expanded} value='3'>
          <ComplementForm />
        </SideAccordion>
      </Accordion>
