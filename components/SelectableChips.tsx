  ...
  return (
    <div className='flex w-full flex-wrap gap-2'>
      {chips.length > 0 ? chips.map((chip) =>
        <div key={chip[chipText]} onClick={() => { selectedChip === chip && chip.ref === selectedChip.ref ? setFunction(undefined) : setFunction(chip) }} className={`flex border transition-colors rounded-lg cursor-pointer h-10 border-secondary gap-1 items-center px-2 ${chip === selectedChip ? 'bg-primary border-primary text-background [&>div]:text-foreground [&>div]:bg-background' : 'hover:bg-secondary/20'}`}>
          {chip[chipText]}
